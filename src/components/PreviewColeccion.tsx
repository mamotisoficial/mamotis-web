    ...
    <section style={{ padding:'6rem 5rem 9rem' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:'4rem' }}>
        ...
        <Link href="/coleccion" style={{ fontFamily:"'DM Mono',monospace", fontSize:'.54rem',
          letterSpacing:'.2em', color:'var(--oro)', textDecoration:'none',
          textTransform:'uppercase', display:'flex', alignItems:'center', gap:'.7rem' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1.6fr 1fr 1fr',
        ...
        {preview.map((p, i) => (
          <Link key={p.id} href={`/coleccion/${p.id}`}
            ...
            style={{ position:'relative', overflow:'hidden', background:'var(--gris)',
              textDecoration:'none', display:'block',
              ...(i===0 ? { gridRow:'1/3' } : {}) }}>
            {/* Placeholder imagen */}
            <div style={{ width:'100%', height:'100%', background:'#0f0c08',
              display:'flex', alignItems:'center', justifyContent:'center' }}>
              <div style={{ textAlign:'center', opacity:.08 }}>
                <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:'4rem', color:'var(--oro)' }}>M</div>
