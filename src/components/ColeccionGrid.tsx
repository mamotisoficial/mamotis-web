...
export default function ColeccionGrid() {
  ...
  return (
    <div>
      <div className="coleccion-filters" style={{
        padding: '2rem 5rem', display: 'flex', alignItems: 'center', gap: '2.5rem',
        ...
        {['Todas', 'Visón', 'Astracán', 'Rex'].map(f => (
          <button key={f} style={{
            ...
            letterSpacing: '.18em', color: f==='Todas' ? 'var(--oro)' : 'var(--crema)',
            opacity: f==='Todas' ? 1 : .38, textTransform: 'uppercase',
            background: 'none', border: 'none',
            borderBottom: f==='Todas' ? '1px solid rgba(201,168,76,.4)' : '1px solid transparent',
            padding: '.4rem 0', cursor: 'pointer',
          }}>{f}</button>
        ))}
        <span className="coleccion-count" style={{
      ...
      <div className="coleccion-grid-inner" style={{
        ...
        {productos.map(p => (
          <Link key={p.id} href={`/coleccion/${p.id}`}
            ...
            <div style={{
              padding: '1.8rem 2rem 2.2rem', borderTop: '1px solid var(--borde)',
              ...
              <span style={{
                fontSize: '1.1rem', color: 'var(--oro)',
                opacity: hovered===p.id ? .7 : 0,
