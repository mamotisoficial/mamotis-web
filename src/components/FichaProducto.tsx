...
export default function FichaProducto({ producto: p }: { producto: Producto }) {
  const msg = encodeURIComponent(`Hola, me interesa el bolso ${p.nombre} (Nº ${p.num}). Quiero saber más.`);
  ...
  return (
    <div className="ficha-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '100vh' }}>
      <div className="ficha-image" style={{
        position: 'relative', overflow: 'hidden', background: '#0a0807',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      }}>
        <div style={{ textAlign: 'center', opacity: .06 }}>
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '14rem', color: 'var(--oro)', lineHeight: 1 }}>M</div>
      ...
      <div className="ficha-info" style={{
        padding: '10rem 5rem 5rem 4.5rem', display: 'flex',
        flexDirection: 'column', justifyContent: 'center', borderLeft: '1px solid var(--borde)',
      }}>
        <Link href="/coleccion" style={{
          ...
          textDecoration: 'none', marginBottom: '3rem', display: 'block',
        }}>
          ← Volver a la colección
        </Link>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.52rem',
          letterSpacing: '.35em', color: 'var(--oro)', opacity: .4, textTransform: 'uppercase', marginBottom: '.8rem' }}>
          Pieza única · Nº {p.num}
        </div>
        <h1 style={{
          fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontStyle: 'italic',
          fontSize: 'clamp(2rem, 4vw, 3.8rem)', lineHeight: 1.08, color: 'var(--crema)', marginBottom: '.5rem',
        }}>
          {p.nombre}
        </h1>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.54rem',
          letterSpacing: '.25em', color: 'var(--oro)', opacity: .55, textTransform: 'uppercase', marginBottom: '3rem' }}>
          {p.lugar}
        </div>
          ...
          {attrs.map(({ label, value }) => (
            <div key={label} className="ficha-attr" style={{
              ...
              <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '.5rem',
                letterSpacing: '.25em', color: 'var(--crema)', opacity: .3, textTransform: 'uppercase' }}>{label}</span>
              <span style={{ fontFamily: "'Cormorant Garamond', serif",
                fontSize: '1.05rem', color: 'var(--crema)', opacity: .8, lineHeight: 1.5 }}>{value}</span>
            </div>
          ))}
        </div>
        <div style={{ width: '100%', height: '1px', background: 'var(--borde)', marginBottom: '2.5rem' }} />
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: '.5rem',
          letterSpacing: '.3em', color: 'var(--oro)', opacity: .4, textTransform: 'uppercase', marginBottom: '1rem' }}>
          Historia del abrigo
        </div>
        <p style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2.5rem' }}>
          ...
          <span style={{ fontFamily: "'DM Mono', monospace", fontSize: '.5rem',
            ...
            Pieza única · Solo existe una unidad
          </span>
        </div>
        <a href={waUrl} target="_blank" rel="noreferrer" style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem',
          padding: '1.3rem 2.5rem', width: '100%', background: 'transparent',
          border: '1px solid rgba(201,168,76,.4)', fontFamily: "'DM Mono', monospace",
          fontSize: '.58rem', letterSpacing: '.22em', color: 'var(--oro)',
          textTransform: 'uppercase', textDecoration: 'none', marginBottom: '1rem',
        }}>
