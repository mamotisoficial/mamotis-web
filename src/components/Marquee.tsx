...
const items = ['Renace', '·', 'MAMOTIS', '·', 'Segunda Vida', '·', 'Irrepetible', '·', 'Una sola', '·'];
...
function Track({ items, reverse }: { items: string[], reverse?: boolean }) {
  ...
  return (
    <div style={{ display: 'flex', whiteSpace: 'nowrap', overflow: 'hidden', padding: '1.6rem 0' }}>
      <div style={{ display: 'flex', flexShrink: 0, animation: `${reverse ? 'mqR' : 'mq'} 30s linear infinite` }}>
        {content}
      </div>
      <div style={{ display: 'flex', flexShrink: 0, animation: `${reverse ? 'mqR' : 'mq'} 30s linear infinite` }} aria-hidden>
        {content}
      </div>
      <style>{`
